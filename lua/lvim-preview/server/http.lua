...
local function serve_doc(client, urlpath)
    ...
    read_file(path, function(data)
        ...
        if kind == "html" then
            -- Serve the page as authored, with the WS client injected so it live-reloads on save.
            return respond(client, "200 OK", "text/html; charset=utf-8", template.inject_reload(data))
        elseif kind then
